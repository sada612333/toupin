...
// ===== Color Schemes =====
// 仅在一处硬编码品牌色，UI 中应当通过 MaterialTheme.colorScheme.* 获取语义化颜色。
...
// ===== 自适应布局尺寸（CompositionLocal，供平板/TV 等不同屏使用） =====
@Immutable
...
@Composable
fun provideAdaptiveDimens(): AdaptiveDimens {
 ...
 * ```
 * ToupinTheme {
 *     Surface(Modifier.fillMaxSize()) { /* your UI */ }
 * }
 * ```
 *
 * 所有颜色、字号、圆角必须通过 [MaterialTheme.colorScheme] / [MaterialTheme.typography]
 * / [MaterialTheme.shapes] 获取，**不要在组件中写死具体数值**。
 */
@Composable
fun ToupinTheme(
